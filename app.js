...
(function () {
    ...
    async function load() {
        ...
        try {
            // 1) Try the live API first — this reflects products added by the owner in real time
            try {
                ...
                console.warn('Live API fetch failed, trying static file:', err);
            }

            // 2) Fallback: the static snapshot file (useful if the API is briefly down)
            if (!data) {
                try {
                    ...
                    console.warn('Static file fetch failed, trying inline fallback:', err);
                }
            }

            // 3) Last resort: read inline JSON (works with file:// protocol)
            if (!data) {
